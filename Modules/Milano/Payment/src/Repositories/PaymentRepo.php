...
class PaymentRepo
{
    ...
    public function searchEmail($email)
    {
        if (!is_null($email)) {
            $this->query->join("users", "payments.buyer_id", 'users.id')
                ->select("payments.*", "users.email")->where("email", "like", "%" . $email . "%");
        }return $this;
    }
    ...
    {
        return  $this->getUserSuccessPayments($userId)->whereDate("created_at", $date)->sum("seller_share");
    }
    ...
    {
        return
            Payment::where("seller_id", $userId)
