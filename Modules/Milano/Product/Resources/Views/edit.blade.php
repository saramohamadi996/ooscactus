...
@section('content')
    <div class="row no-gutters  ">
        <div class="col-12 bg-white">
            ...
            <form action="{{ route('products.update', $product->id) }}" class="padding-30" method="post"
...
{{--                             class="text"/>--}}
                ...
                <div class="d-flex multi-text">
                    ...
                    <x-input type="number" placeholder="درصد فروشنده" name="seller_share"
                             class="text-left" value="{{ $product->seller_share }}"/>

                    <x-input type="number" placeholder="موجودی انبار" name="stock"
                             class="text-left" value="{{ $product->stock }}"/>

                    <x-input type="string" placeholder="کد محصول" name="code_product"
                             class="text-left" value="{{ $product->code_product }}"/>
                </div>
                    ...
                    <x-select name="seller_id">
                        ...
                        @foreach($sellers as $seller)
                            <option value="{{ $seller->id }}"
                                    @if($seller->id == $product->seller_id) selected @endif>{{ $seller->name }}</option>
                        @endforeach
                    </x-select>
                        ...
                        <div class="col-12" style="margin-top: 20px">
                            @foreach ($product->images as $image)
                                ...
                                <input type="radio" data-toggle="tooltip" data-placement="bottom"
                                       data-original-title="انتخاب به عنوان عکس شاخص" name="main_image" id=""
                                       class="ml-3" value="{{$image->src}}"
                                       @if($image->main_image == 1) checked @endif>
                            @endforeach
                        </div>
                    </div>
                </div>
                <input type="file" name="images[]" oninvalid="this.setCustomValidity('فیلد تصاویر محصول الزامی است')"
                       multiple placeholder="تصویر محصول">
